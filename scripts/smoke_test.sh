...
JOB_ID=$(curl -s -X POST http://localhost:8080/jobs -d "$JOB_JSON" | sed -E 's/.*"job_id":"([^"]+)".*/\1/')
[ -n "$JOB_ID" ] || { echo "[smoke] Failed to parse job id"; exit 1; }
echo "[smoke] Created job $JOB_ID"

echo "[smoke] Enqueue run"
curl -s -X POST http://localhost:8080/jobs/$JOB_ID/run || true
...
echo "[smoke] Metrics sample:"; curl -s http://localhost:8080/metrics | head -n 10
